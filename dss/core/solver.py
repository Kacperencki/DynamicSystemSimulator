...
class Solver:
    ...
    def __init__(self, system, initial_conditions,T=5, fps=60 ,t_span=None, t_eval=None):
        self.system = system
    ...
    def run(self): # RETURNS STATE VARIABLE
        try:
            sol = scipy.integrate.solve_ivp(self.system.dynamics, t_span=self.t_span,t_eval=self.t_eval, y0= self.initial_cond, method="RK45", rtol=1e-4, atol=1e-6)
