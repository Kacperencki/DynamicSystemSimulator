...
def controls(prefix: str) -> Controls:
    ...
    with st.form(key=f"{prefix}_form"):
        ...
        with st.expander("Motor parameters", expanded=False):
            ...
            with c1:
                R = st.number_input("R [Ω]", value=2.0, min_value=0.0, format="%.6g", key=f"{prefix}_R")
                L = st.number_input("L [H]", value=1e-3, min_value=0.0, format="%.6g", key=f"{prefix}_L")
            with c2:
                Ke = st.number_input("K_e [V/(rad/s)]", value=0.1, min_value=0.0, format="%.6g", key=f"{prefix}_Ke")
                Kt = st.number_input("K_t [N·m/A]", value=0.1, min_value=0.0, format="%.6g", key=f"{prefix}_Kt")
            with c3:
                J = st.number_input("J [kg·m²]", value=1e-3, min_value=0.0, format="%.6g", key=f"{prefix}_J")
                bm = st.number_input("b_m [N·m·s]", value=1e-4, min_value=0.0, format="%.6g", key=f"{prefix}_bm")

        with st.expander("Input voltage parameters", expanded=False):
            c1, c2, c3 = st.columns(3)
            with c1:
                V0 = st.number_input("V0 [V]", value=12.0, format="%.6g", key=f"{prefix}_V0")
            with c2:
                v_offset = st.number_input("Offset [V]", value=0.0, format="%.6g", key=f"{prefix}_v_offset")
            with c3:
                t_step = st.number_input("t_step [s]", value=0.05, min_value=0.0, format="%.6g", key=f"{prefix}_t_step")

            if v_mode in ("sine", "square"):
                d1, d2 = st.columns(2)
                with d1:
                    v_freq = st.number_input("f [Hz]", value=2.0, min_value=0.0, format="%.6g", key=f"{prefix}_v_freq")
                with d2:
                    v_duty = st.number_input("Duty (square)", value=0.5, min_value=0.0, max_value=1.0, format="%.3f", key=f"{prefix}_v_duty")
            else:
        ...
        with st.expander("Load parameters", expanded=False):
            if load_mode == "none":
                ...
                tau_load = st.number_input("τ_load [N·m]", value=0.05, min_value=0.0, format="%.6g", key=f"{prefix}_tau_load")
                b_load = 0.0
                tau_c = 0.0
                omega_eps = float(st.session_state.get(f"{prefix}_omega_eps", 0.5))
            elif load_mode == "viscous":
                b_load = st.number_input("b_load [N·m·s]", value=2e-3, min_value=0.0, format="%.6g", key=f"{prefix}_b_load")
                tau_load = 0.0
                tau_c = 0.0
                omega_eps = float(st.session_state.get(f"{prefix}_omega_eps", 0.5))
            else:  # coulomb
                tau_c = st.number_input("τ_c [N·m]", value=0.02, min_value=0.0, format="%.6g", key=f"{prefix}_tau_c")
                omega_eps = st.number_input("ω_eps [rad/s]", value=0.5, min_value=1e-6, format="%.6g", key=f"{prefix}_omega_eps")
                tau_load = 0.0
                b_load = 0.0
        ...
        with st.expander("Initial state", expanded=False):
            c1, c2 = st.columns(2)
            with c1:
                i0 = st.number_input("i(0) [A]", value=0.0, format="%.6g", key=f"{prefix}_i0")
            with c2:
                omega0 = st.number_input("ω(0) [rad/s]", value=0.0, format="%.6g", key=f"{prefix}_omega0")
        ...
        perf = animation_performance(
            ...
            trail_default=False,
            trail_checkbox_label="Show tail",
