...
class CompositionConfigKwds(TypedDict, total=False):
    ...
    spacing
        The default spacing in pixels between composed sub-systems.
...
class ProjectionKwds(TypedDict, total=False):
    ...
    lobes
        The number of lobes in projections that support multi-lobe systems: ``berghaus``,
...
class TitleParamsKwds(TypedDict, total=False):
    ...
    anchor
        ...
        **Default value:** ``"middle"`` for `single
        ...
        <https://vega.github.io/vega-lite/docs/layer.html>`__ systems. ``"start"`` for other
        composite systems.

        **Note:** `For now <https://github.com/vega/vega-lite/issues/2875>`__, ``anchor`` is
        only customizable only for `single
        <https://vega.github.io/vega-lite/docs/spec.html>`__ and `layered
        <https://vega.github.io/vega-lite/docs/layer.html>`__ systems. For other composite
        systems, ``anchor`` is always ``"start"``.
    angle
        Angle in degrees of title and subtitle text.
    aria
        A boolean flag indicating if `ARIA attributes
...
class TopLevelSelectionParameterKwds(TypedDict, total=False):
    ...
    views
        ...
        this property is specified, selections will only be applied to systems with the given
        names.
    """
...
class ThemeConfig(TypedDict, total=False):
    ...
    spacing
        The spacing in pixels between sub-systems of the composition operator. An object of
